...
class State(Enum):
    HOME = 0    #state in which Token is in Home, Starting position.
    SAFE = 1    #State in which Token has reached the goal.
    UNSAFE = 2  #State where Token is safe but not home or goal
...
class statespace():
        ...
        for player in players:
            ...
            for piece in player.pieces:
                if piece == 0:
                    ...
                    self.global_player_position[idx_player].pieces[idx_piece] = (piece + (
                                self.quarter_game_size * idx_player)) % 52
    ...
    def check_if_piece_safe(self, player, piece):
        is_protected = len(
            np.where(self.local_player_position[player].pieces == self.local_position(player, piece))[0]) > 1
        return self.check_if_piece_is_safe_at_location(self.local_position(player, piece),
                                                       self.global_position(player, piece), is_protected)
    ...
    def check_if_piece_is_in_danger(self, player, piece):
        return self.check_if_piece_is_in_danger_at_location(self.local_position(player, piece),
                                                            self.global_position(player, piece))
